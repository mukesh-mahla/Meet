...
import './App.css';
  ...
  async function startScreenShare() {
    ...
    // replace the camera video track with screen track on every peer connection
    pcsRef.current.forEach(pc => {
      const sender = pc.getSenders().find(s => s.track?.kind === "video");
      if (sender) sender.replaceTrack(screenTrack);
    });

    // show your own screen in your local video too
    videoRef.current.srcObject = screenStream;

    // when user clicks "stop sharing" in browser UI, switch back to camera
